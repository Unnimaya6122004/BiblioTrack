...
export default function QuickActions() {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_14px_28px_-24px_rgba(15,23,42,0.8)]">
      ...
      <div className="pointer-events-none absolute -right-8 -top-10 h-24 w-24 rounded-full bg-cyan-100/60 blur-xl" />
      <div className="pointer-events-none absolute -bottom-12 left-0 h-20 w-20 rounded-full bg-indigo-100/70 blur-xl" />
        ...
        {actions.map((action) => (
          <li key={action.label}>
            ...
            <button
              ...
              className="group flex w-full items-center justify-between rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-2.5 text-left transition hover:border-cyan-200 hover:bg-cyan-50/70"
            >
              ...
              <ArrowRight
                size={16}
                className="shrink-0 text-slate-400 transition group-hover:translate-x-0.5 group-hover:text-cyan-600"
              />
            </button>
